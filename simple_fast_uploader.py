...
async def main():
    ...
    try:
        ...
        try:
            # حاول كرقم
            numbers = [s for s in channel_username.split() if s.startswith('-100')]
            if numbers:
                entity = await client.get_entity(int(numbers[0]))
        except:
            try:
                # حاول في الدردشات
                async for dialog in client.iter_dialogs(limit=10):
                    if hasattr(dialog.entity, 'id') and str(dialog.entity.id) in channel_username:
